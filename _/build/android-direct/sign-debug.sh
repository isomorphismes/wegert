...
input=${1:-"$build_root/build/direct/wegert-direct-unsigned.apk"}
output=${2:-"$build_root/build/direct/wegert-direct-debug.apk"}
android_home=${ANDROID_HOME:-${ANDROID_SDK_ROOT:-}}
build_tools=${ANDROID_BUILD_TOOLS:-}

[[ -f $input ]] || { echo "missing unsigned APK: $input" >&2; exit 1; }
[[ -f $build_root/app/wegert-debug.keystore ]] || { echo "missing public debug keystore" >&2; exit 1; }
...
"$apksigner" sign \
  --ks "$build_root/app/wegert-debug.keystore" \
  --ks-type PKCS12 \
