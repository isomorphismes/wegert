...
`gitlab-status.grease` searches the public `fdroid/fdroiddata` merge requests for **zero & infinity** from the submission branch, including closed and merged MRs. It reports whether the MR was accepted, merge/pipeline state, labels, dates, and non-system comments from GitLab's public merge-request Notes API. It checks source visibility against the `SourceCode` URL in the F-Droid metadata, not the submitter's fdroiddata fork, so neither check requires access to that fork or a GitLab token. The source branch defaults to `master`; pass another branch as the first argument or set `GITLAB_BRANCH`. Override the search text with `GITLAB_MR_SEARCH`.
