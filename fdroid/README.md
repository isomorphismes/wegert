...
`gitlab-status.grease` searches the public `fdroid/fdroiddata` merge requests for **zero & infinity** from the submission branch, including closed and merged MRs. It reports whether the MR was accepted, merge/pipeline state, labels, dates, and non-system reviewer comments. Comments are read from GitLab's public GraphQL discussions, so the check does not require access to the submitter's fork. The source branch defaults to `master`; pass another branch as the first argument or set `GITLAB_BRANCH`. Override the search text with `GITLAB_MR_SEARCH`. `GITLAB_TOKEN` is optional and is used when present.
