...
The checked-in rational-function MP4 bytes predate the renderer refactor and were generated by the earlier NumPy/Pillow implementation. This refactor does not silently relabel those existing artifacts.

The current regeneration path for the six rational-function movies is now `_/build/render_root_videos.py` -> `code/wegert_render_frames.c` -> the same `wegert_portrait_renderer_gles` and GLSL colour core used by the Android portrait. The Python script chooses factor positions and passes scene descriptions to the C renderer; it no longer independently evaluates the rational function or implements Wegert colouring. FFmpeg remains the video encoder. Once those MP4s are explicitly regenerated, their new bytes can be attributed to this canonical path.

`rendered_images/wegert_meromorphic_15s.mp4` remains a legacy generated output from the earlier NumPy/Pillow script. It included an `exp(0.18 z)` factor and a separate `dark_rgb` colouring rule, neither of which belongs to the current rational `wegert_scene` contract. The refactored generator therefore does not regenerate that file rather than pretending it is covered by the canonical rational renderer.
