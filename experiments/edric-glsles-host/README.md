...
7. `wegert_idric_compile` — compile `WegertProbe.idric`, which exercises the shared 64-zero/64-pole Wegert phase portrait.
8. `generated_glsl_validate` — validate and link the generated fragment against the backend's Wegert fullscreen vertex fixture.
...
- Edric `main` + GLSL backend `main`;
- Edric `main` + the Mali-G57 backend branch with `float-precision=mediump`;
- the no-wide-float Edric branch + that same Mali branch.

The last lane is expected to tell us whether the remaining boundary is the compiler hookup or the shader source scalar vocabulary. At present `Shader.Source` still uses inherited `Double` names even though the emitted GLSL value is `float`. If the no-wide-float source profile rejects the probe before shader lowering, the receipt should classify that specifically as `SOURCE_FLOAT_PROFILE_BLOCKS_SHADER_API` rather than as a generic GLSL failure.
